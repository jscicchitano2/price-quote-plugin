...
class Price_Quote_Form
{
  ...
  const FORM_LAST        = '<div class="form last-form style="display:none;"><h2>Your %s Total</h2><div class="form-text"><p id="total-cost"></p><p id="total-description"></p><p><button class="form-button" type="button">Schedule Now</button></p></div></div>';
  ...
  function __construct()
  {
    ...
    // Register form post type and assets
    add_action( 'init', array( $this, 'register_form_post_type' ) );
    add_action( 'wp_enqueue_scripts', array( $this, 'register_form_assets' ) );

    // Register form response post type
  ...
  // Include plugin JS dependencies
  function register_form_assets()
  {
    wp_register_script( 'jquery', plugin_dir_url( __FILE__ ) . 'js/libraries/jquery.js' );
          ...
          while ( have_rows( 'form', $post_id ) ) : the_row();
            ...
            switch ( $question ) {
              case "text_input_layout":
                ...
                $question_text = get_sub_field('text_question');
                $name = strtolower(preg_replace('/[^A-Za-z0-9-]+/', '-', $title));
                $questions[] = array($name, $question_text);
                break;
              case "radio_input_layout":
                $title = get_sub_field('radio_title');
                $question_text = get_sub_field('radio_question');
                $name = strtolower(preg_replace('/[^A-Za-z0-9-]+/', '-', $title));
                $questions[] = array($name, $question_text);
                break;
              case "checkbox_input_layout":
                $title = get_sub_field('checkbox_title');
                $question_text = get_sub_field('checkbox_question');
                $name = strtolower(preg_replace('/[^A-Za-z0-9-]+/', '-', $title));
                $questions[] = array($name, $question_text);
                break;
            }
  ...
  function shortcode( $atts )
  {
    ...
    $formreturn = '<div id="form-outer-div" data-title="' . $title . '" data-scoring="' . $scoring . '" class="form-template-' . $atts->id . '"><form id="price-quote-form" action="" method="post" enctype="multipart/form-data">';
    ...
    while ( have_rows( 'form', $id ) ) : the_row();
      ...
      switch ( $layout ) {
        case "text_input_layout":
          ...
          $text_question = get_sub_field('text_question');
          $text_placeholder = get_sub_firld('text_placeholder');
          $text_scoring = get_sub_field('text_scoring');
          $name = strtolower(preg_replace('/[^A-Za-z0-9-]+/', '-', $text_title));
        ...
        case "radio_input_layout":
            $form_count += 1;
            $radio_title = get_sub_field('radio_title');
            $radio_question = get_sub_field('radio_question');
            $radio_scoring = get_sub_field('radio_scoring');
            $name = strtolower(preg_replace('/[^A-Za-z0-9-]+/', '-', $radio_title));
            ...
            foreach ($scores as $score) {
                if (strpos($score, ' | ') !== false) {
                    ...
                    $selectreturn .= '<label class="label" for="' . $option_id . '"><input type="radio" id="' . $option_id . '" name="' . $name . '" value="' . $score[0] . '" /><b>' . $score[0] . '</b></label>';
                    $option_count = $option_count + 1;
                }
            }
            $scoring = htmlspecialchars(json_encode($scoring));
            $first = false;
            $formreturn .= sprintf( self::FORM_TEXT, $form_count, $visibility, $scoring, $radio_title, $radio_question, $type, $name, $radio_placeholder);
            break;
        case "checkbox_input_layout":
            $form_count += 1;
            $checkbox_title = get_sub_field('checkbox_title');
            $checkbox_question = get_sub_field('checkbox_question');
            $checkbox_scoring = get_sub_field('checkbox_scoring');
            $name = strtolower(preg_replace('/[^A-Za-z0-9-]+/', '-', $checkbox_title));
            ...
            $scoring = htmlspecialchars(json_encode($scoring));
            $first = false;
            $formreturn .= sprintf( self::FORM_TEXT, $form_count, $visibility, $scoring, $checkbox_title, $checkbox_question, $type, $name, $checkbox_placeholder);
            break;
        break;
      }
    endwhile;
  ...
  function set_form() 
  {
    ...
    $post_title = explode("@", $responses[$keys[3]][1]);
   ...
   foreach ($keys as $key) {
    if ($key != 'formTitle' && $key != 'scoreTotal' && $key != 'priceTotal') {
